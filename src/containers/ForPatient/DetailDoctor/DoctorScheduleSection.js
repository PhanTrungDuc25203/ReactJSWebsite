  ...
  async componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.selectedDoctorId !== this.props.selectedDoctorId) {
      ...
      if (tempDateArr && tempDateArr.length > 0) {
        let res = await getDoctorScheduleByDateService(
  ...
  handleChoosingATimeframeForAppointment = (doctor) => {
    ...
    if (this.props.language === LANGUAGES.VI) {
      formattedDate = this.capitalizeFirstLetter(
        moment(doctor.date).format("dddd, DD-MM-YYYY")
      );
    }
    if (this.props.language === LANGUAGES.EN) {
      formattedDate = moment(doctor.date)
        .locale("en")
    ...
    this.props.history.push(
      `/make-appointment/${doctor.doctorId}/${formattedDate}/${doctor.timeType}`
    );
              ...
              allAvailableTimeframe.map((item, index) => {
                let timeframe =
                  language === LANGUAGES.VI
                    ? item.timeTypeData.value_Vie
                    : item.timeTypeData.value_Eng;
                return (
                  <button
                    key={index}
                    className="timeframe-button"
                    onClick={() =>
                      this.handleChoosingATimeframeForAppointment(item)
                    }
                  >
                    {timeframe}
                  </button>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(DoctorScheduleSection)
);
