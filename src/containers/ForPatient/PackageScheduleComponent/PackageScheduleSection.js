...
class PackageScheduleSection extends Component {
  ...
  async componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.selectedPackageId !== this.props.selectedPackageId) {
      ...
      if (tempDateArr && tempDateArr.length > 0) {
        let res = await getPackageScheduleByDateService(
  ...
  handleChoosingATimeframeForPeriodicExam = (pack) => {
    ...
    if (this.props.language === LANGUAGES.VI) {
      formattedDate = this.capitalizeFirstLetter(
        moment(pack.date).format("dddd, DD-MM-YYYY")
      );
    }
    ...
    this.props.history.push(
      `/booking-a-exam-package/${pack.examPackageId}/${formattedDate}/${pack.timeType}`
    );
  };
    ...
    return (
      <React.Fragment>
        <div className="available-day">
          <select
            className="available-day-selection"
          ...
          <div className="timeframe-selection">
            {allAvailableTimeframe && allAvailableTimeframe.length > 0 ? (
              allAvailableTimeframe.map((item, index) => {
                let timeframe =
                  language === LANGUAGES.VI
                    ? item.timeTypeDataForPackage.value_Vie
                    : item.timeTypeDataForPackage.value_Eng;
                return (
                  <button
                    key={index}
                    className="timeframe-button"
                    onClick={() =>
                      this.handleChoosingATimeframeForPeriodicExam(item)
                    }
                  >
                    {timeframe}
                  </button>
              ...
              <div className="sorry-text-because-noavailable-timeframe">
                Xin lỗi quý khách, gói khám này đã kín lịch đăng ký!
              </div>
            )}
          </div>
        </div>
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(PackageScheduleSection)
);
