...
class DetailMedicalFacility extends Component {
  ...
  async componentDidMount() {
    ...
    const sections = document.querySelectorAll(
      "#booking-appointment, #general-introduction, #equipments, #specialty-area, #location, #exam-process"
    );
  ...
  initializeMap = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        L.marker([lat, lon])
          .addTo(this.map)
          .bindPopup(`<b>${medicalFacility.name}</b><br>${address}`)
          .openPopup();
      } else {
  ...
  render() {
    ...
    if (medicalFacility && medicalFacility.image) {
      imageByBase64 = Buffer.from(medicalFacility.image, "base64").toString(
        "binary"
      );
    }
    return (
          ...
          <div className="medical-facility-article-container">
            <div className="medical-facility-background-and-avatar-image">
              ...
              <div className="medical-facility-avatar-and-name">
                <div
                  className={
                    imageByBase64 ? "avatar-css" : "default-avatar-css"
                  }
                  style={{
                    backgroundImage: `url(${
                      imageByBase64 ? imageByBase64 : defaultMedicalFacility
                    })`,
                  }}
                ></div>
            ...
            <div className="medical-facility-scrollspy-menu">
              <header className="scrollspy-menu-header">
                <nav className="facility-scrollspy-menu-navigation">
                  <a
                    onClick={() => this.scrollToSection("booking-appointment")}
                    className={
                      activeSection === "booking-appointment" ? "active" : ""
                    }
                  >
                    Đặt lịch khám
                  </a>
                  <a
                    onClick={() => this.scrollToSection("general-introduction")}
                    className={
                      activeSection === "general-introduction" ? "active" : ""
                    }
                  >
                    Giới thiệu chung
                  </a>
                  <a
                    onClick={() => this.scrollToSection("equipments")}
                    className={activeSection === "equipments" ? "active" : ""}
                  >
                    Trang thiết bị
                  </a>
                  <a
                    onClick={() => this.scrollToSection("specialty-area")}
                    className={
                      activeSection === "specialty-area" ? "active" : ""
                    }
                  >
                    Chuyên ngành
                  </a>
                  <a
                    onClick={() => this.scrollToSection("location")}
                    className={activeSection === "location" ? "active" : ""}
                  >
                    Vị trí & Bản đồ
                  </a>
                  <a
                    onClick={() => this.scrollToSection("exam-process")}
                    className={activeSection === "exam-process" ? "active" : ""}
                  >
                    Quy trình khám
                  </a>
                </nav>
            ...
            <div
              ...
              className="medical-facility-booking-with-doctor-section"
            >
              {medicalFacility.medicalFacilityDoctorAndSpecialty &&
                medicalFacility.medicalFacilityDoctorAndSpecialty.length >
                  0 && (
                  <label className="section-label">
                    Các bác sĩ tại Cơ sở y tế
                  </label>
                )}
              {medicalFacility.medicalFacilityDoctorAndSpecialty &&
                medicalFacility.medicalFacilityDoctorAndSpecialty.length > 0 &&
                medicalFacility.medicalFacilityDoctorAndSpecialty.map(
                  (item, index) => {
                    return (
                      <div
                        className="doctors-of-this-medical-facility"
                        key={index}
                      >
            ...
            <div className="medical-facility-booking-exam-package-section">
              {medicalFacility.medicalFacilityExamPackage &&
                medicalFacility.medicalFacilityExamPackage.length > 0 && (
                  <label className="section-label">
                    Các gói khám của Cơ sở y tế
                  </label>
                )}
            </div>
            ...
            <div
              ...
              className="medical-facility-introduction-section"
            >
            ...
            <div
              id="equipments"
              className="medical-facility-equipments-section"
            >
              <label className="section-label">
            ...
            <div
              id="specialty-area"
              className="medical-facility-specialty-area-section"
            >
              <label className="section-label">Thế mạnh chuyên môn</label>
              {medicalFacility.medicalFacilitySpecialtyData &&
              medicalFacility.medicalFacilitySpecialtyData.length > 0 ? (
                <ul>
                  {medicalFacility.medicalFacilitySpecialtyData.map(
                    (item, index) => (
                      <li key={index}>
