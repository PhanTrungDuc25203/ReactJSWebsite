...
class DoctorManage extends Component {
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    if (prevProps.allRequiredDoctorData !== this.props.allRequiredDoctorData) {
      let { resPaymentMethod, resPrice, resProvince, resSpecialty } =
        this.props.allRequiredDoctorData;
      let selectPriceData = this.buildDataForDoctorSelectBox(
        resPrice,
        "priceSelection"
      );
      let selectPaymentMethodData = this.buildDataForDoctorSelectBox(
        resPaymentMethod,
        "paymentMethodSelection"
      );
      let selectProvinceData = this.buildDataForDoctorSelectBox(
        resProvince,
        "provinceSelection"
      );
      let selectSpecialtyData = this.buildDataForDoctorSelectBox(
        resSpecialty,
        "specialtySection"
      );
      this.setState({
    ...
    if (prevProps.language !== this.props.language) {
      ...
      let { resPaymentMethod, resPrice, resProvince } =
        this.props.allRequiredDoctorData;
      let selectPriceData = this.buildDataForDoctorSelectBox(
        resPrice,
        "priceSelection"
    ...
    if (prevProps.medicalFacility !== this.props.medicalFacility) {
      let selectData = this.buildDataForDoctorSelectBox(
        this.props.medicalFacility,
        "medicalFacilitySelection"
      );
      this.setState({
  ...
  handleSaveMarkdownContent = () => {
    ...
    this.props.saveDoctorDetails({
      ...
      action:
        hadOldDataForEdit === true ? CRUD_ACTIONS.EDIT : CRUD_ACTIONS.CREATE,
  ...
  handleChangeOnSelectBox = async (selectedDoctor) => {
    ...
    await this.props.fetchDoctorDetailsForDoctorManagePage(
    ...
    if (selectedDoctorDetails && selectedDoctorDetails.ArticleMarkdown) {
      ...
      let tempClinicAddress = "",
        ...
        tempSpecialtyId = "",
        tempSelectedSpecialty = "",
        tempMedicalFacilityId = "",
        tempSelectedMedicalFacility = "";
      ...
      if (selectedDoctorDetails.Doctor_infor) {
        ...
        tempMedicalFacilityId =
          selectedDoctorDetails?.Doctor_specialty_medicalFacility
            ?.ComplexMedicalFacility?.id;
      ...
      this.setState({
        ...
        selectedMedicalFacility: tempSelectedMedicalFacility
          ? tempSelectedMedicalFacility
          : "",
      });
    } else {
  ...
  buildDataForDoctorSelectBox = (data, isBuiltFor) => {
    ...
    if (data && data.length > 0) {
      ...
      if (
        isBuiltFor === "paymentMethodSelection" ||
        isBuiltFor === "provinceSelection"
      ) {
        data.map((item, index) => {
          let tempObj = {};
        ...
        <div className="header-article-container">
          ...
          <div className="option-section">
            <button
              className={
                hadOldDataForEdit === true
                  ? "save-changes-of-doctor-article-button"
                  : "save-doctor-article-button"
              }
              onClick={() => this.handleSaveMarkdownContent()}
            >
              {hadOldDataForEdit === true ? (
                <span>
                  <FormattedMessage id="doctor-manage-page-for-admin.save-changes-button" />
                </span>
              ) : (
                <span>
                  <FormattedMessage id="doctor-manage-page-for-admin.save-article-button" />
                </span>
              )}
            </button>
            <Select
              ...
              placeholder={
                this.state.isSelectDisabled ? (
                  ...
                  <FormattedMessage id="doctor-manage-page-for-admin.select-doctor-placeholder" />
                )
              }
          ...
          <div className="row row-in-form">
            ...
            <div className="col-md-3 mb-3">
              ...
              <input
                ...
                required
                onChange={(event) =>
                  this.handleOnChangeText(event, "clinicName")
                }
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchDoctorDetailsForDoctorManagePage: (id) =>
      dispatch(actions.fetchDoctorDetailsForDoctorManagePage(id)),
    getBriefInfoOfMedicalFaclityAction: (id) =>
      dispatch(actions.getBriefInfoOfMedicalFaclityAction(id)),
    getRequiredDataForDoctorArticleManagePage: () =>
      dispatch(actions.getRequiredDataForDoctorArticleManagePage()),
  };
};
