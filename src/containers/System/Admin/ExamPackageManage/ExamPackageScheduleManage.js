...
class ExamPackageScheduleManage extends Component {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (prevProps.allPackagesData !== this.props.allPackagesData) {
            let selectData = this.buildDataForPackageSelectBox(this.props.allPackagesData);
            this.setState({
    ...
    render() {
        ...
        return (
            <div className="schedule-for-packagemanage-page-container">
                <div className="schedule-for-packagepage-title">Thời gian biểu và lịch khám của Gói khám</div>
                            ...
                            <div className="col-md-12 choose-time-frame">
                                {timeframe &&
                                    ...
                                    timeframe.map((item, index) => {
                                        return (
                                            <button key={index} className={item.isSelected === true ? "time-frame-button active" : "time-frame-button"} onClick={() => this.handleTimeFrameClicked(item)}>
