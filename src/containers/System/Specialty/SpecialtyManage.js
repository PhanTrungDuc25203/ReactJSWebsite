    ...
    render() {
        ...
        return (
            <div className="specialty-manage-container">
                <div className="specialty-manage-title">Quản lý chuyên khoa</div>
                <div className="required-information">
                    <div className="row">
                        <div className="specialty-name-section col-9 form-group">
                            <label>Tên chuyên khoa</label>
                            <input className="form-control" type="text" value={this.state.name} onChange={(event) => this.handleOnchangeInput(event, "name")}></input>
                        </div>
                        <div className="specialty-image-section col-3 form-group">
                            <label className="name">Ảnh của chuyên khoa</label>
                            <input id="customFileInput" className="form-control-file" type="file" onChange={(event) => this.handleOnChangeImage(event)}></input>
                            {imageSelected && <div className="image-preview image-css" style={{ backgroundImage: `url(${this.state.imageBase64})` }} onClick={() => this.openImagePreview()}></div>}
                        ...
                        <div className="specialty-description-section col-12">
                            <MdEditor style={{ height: "500px" }} renderHTML={(text) => mdParser.render(text)} onChange={this.handleEditorChange} value={this.state.markdownDescription} />
                        </div>
                        <div className="col-9 form-group"></div>
                        <div className="col-3 form-group">
                            <button className="save-button" onClick={() => this.handleSaveNewSpecialty()}>
                                Thêm mới
                            </button>
                        </div>
                    </div>
