...
const AppointmentItemForPatientInfterface = lazy(() =>
  import("./AppointmentItemForPatientInfterface")
);
const AppointmentItemForDoctorInfterface = lazy(() =>
  import("./AppointmentItemForDoctorInfterface")
);
const HistoryOfDoctorAppointment = lazy(() =>
  import("./HistoryOfDoctorAppointment")
);
const HistoryOfPatientAppointment = lazy(() =>
  import("./HistoryOfPatientAppointment")
);
...
class AppointmentInProfilePage extends PureComponent {
  ...
  async componentDidMount() {
    ...
    if (userRole) {
      this.setState({ userRole, currentUserEmail }, async () => {
        let res = await getAllRelativeBookingsOfCurrentSystemUserService(
  ...
  handleHistoryOrHandlingButtonClicked = async () => {
    this.setState((prevState) => ({
      historyOrHandling:
        prevState.historyOrHandling === "history" ? "handling" : "history",
    }));
    let res = await getAllRelativeBookingsOfCurrentSystemUserService(
      this.state.currentUserEmail
    );
    ...
    return (
      <div className="appointment-container">
        {historyOrHandling === "handling" ? (
                    ...
                    {isDoctor ? (
                      <AppointmentItemForDoctorInfterface
                        ...
                        appointmentTimeFrame={
                          item.appointmentTimeTypeData?.value_Vie
                        }
                        patientBirthday={moment(item.patientBirthday).format(
                          "DD-MM-YYYY"
                        )}
                      />
                    ) : (
          ...
          <Suspense fallback={<div>Loading...</div>}>
            {isDoctor ? (
              <HistoryOfDoctorAppointment currentUserEmail={currentUserEmail} />
            ) : (
      ...
      <div className="appointment-in-profile-page">
        <div className="appointment-in-profile-page-header">
          ...
          <a
            // href="#"
            className={
              historyOrHandling === "history" ? "btn-flip-backward" : "btn-flip"
            }
            data-back={
              historyOrHandling === "history" ? "Chưa khám" : "Lịch sử"
            }
            data-front={
              historyOrHandling === "history" ? "Lịch sử" : "Chưa khám"
            }
            onClick={this.handleHistoryOrHandlingButtonClicked}
          />
        </div>
        <div className="appointment-container">
          {combinedAppointments.patientAppointments &&
          combinedAppointments.patientAppointments.length > 0
          ...
          {combinedAppointments.doctorAppointments &&
          combinedAppointments.doctorAppointments.length > 0
            ? this.renderAppointmentItems(
                combinedAppointments.doctorAppointments,
                true
              )
...
export default withRouter(
  connect(mapStateToProps, mapDispatchToProps)(AppointmentInProfilePage)
);
