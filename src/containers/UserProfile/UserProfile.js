...
const AppointmentInProfilePage = lazy(() =>
  import("./Appointment/AppointmentInProfilePage")
);
...
class UserProfile extends Component {
  ...
  async componentDidMount() {
    if (
      ...
      if (
        res &&
        res.errCode === 0 &&
        !_.isEqual(res.data, this.state.currentUser)
      ) {
        this.setState({
  ...
  renderCommentSection = () => {
    ...
    if (commentAboutDoctorsOpended) {
      return (
        <div className="personal-profile">Your comments about doctors</div>
      );
    }
    return null;
  };
        ...
        <CustomScrollbars style={{ height: "100vh", width: "100%" }}>
          ...
          <div className="content-container">
            <div className="nav-bar">
              <a
                onClick={() =>
                  this.handleProfileTabClicked("personalProfileOpen")
                }
                className={this.state.personalProfileOpened ? "active" : ""}
              >
                Trang cá nhân
              </a>
              <a
                onClick={() => this.handleProfileTabClicked("appointmentOpen")}
                className={this.state.appointmentOpened ? "active" : ""}
              >
                Lịch hẹn bác sĩ
              </a>
              <a
                onClick={() =>
                  this.handleProfileTabClicked("commentAboutDoctorsOpen")
                }
                className={
                  this.state.commentAboutDoctorsOpended ? "active" : ""
                }
              >
                Nhận xét về bác sĩ
              </a>
              <a
                onClick={() => this.handleProfileTabClicked("returnToHomePage")}
                className={this.state.returnToHomePage ? "active" : ""}
              >
            ...
            <div className="logout-button-container">
              <button
                onClick={this.handleLoginForUser}
                className="log-out-button-of-profile-page"
              >
